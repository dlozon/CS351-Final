...
from functions import current_time_millis, formatedDate
...
def chatroom():
	if request.cookies.get('logged_in_temp') or request.cookies.get('logged_in_perm'):
		# obtain username from cookies
		username = request.cookies.get('logged_in_temp')
		if not username:
			username = request.cookies.get('logged_in_perm')
		...
		if request.method == 'GET':
			...
			if response['Items']:
				response['Items'].sort(key=lambda x: x["time_sent"]['N'], reverse=False)
			return render_template('chatroom.html', page_name='chatroom', username=username, messages=response['Items'])
		else:
			# if user is logged in and sending a message, put message in db
			message = request.form['message']
			time = str(current_time_millis())
		...
		if response['Items']:
			response['Items'].sort(key=lambda x: x["time_sent"]['N'], reverse=False)
			return render_template('chatroom.html', page_name='chatroom', messages=response['Items'])
		...
		if 'Item' in response:
			...
			if bcrypt.checkpw(request.form['password'].encode('utf-8'), response['Item']['password']['B']):
				...
				if remember:
					...
					resp.set_cookie('logged_in_perm', username, secure=True, httponly=True, max_age=2592000)
					return resp
				else:
					...
					resp.set_cookie('logged_in_temp', username, secure=True, httponly=True)
					return resp
			else:
...
if __name__ == "__main__":
	# app.run(debug=True)
	# For production, use:
	app.jinja_env.globals.update(formatedDate=formatedDate)
